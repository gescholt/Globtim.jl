...
Parse a string representation of a rational number in the format "a/2^b" to a Float64.

# Arguments
- `str::AbstractString`: String in the format "a/2^b"

# Returns
- Float64 value of the parsed rational
"""
...
"""
    process_output_file(file_path::String; dim::Int = 3)

Parse the output file generated by `msolve`.

# Arguments
- `file_path::String`: Path to the output file
- `dim::Int`: Dimension of the points (default: 3)

# Returns
- Vector of Vector{Float64} containing the parsed points
"""
function process_output_file(file_path::String; dim::Int=3)
    ...
    # Use a local variable for last_val to avoid global variable issues
    local last_val = 0.0
    ...
    for m in eachmatch(pair_pattern, content)
        ...
        for i = 1:2
            ...
            if i == 1
                ...
                push!(current_point, (val + last_val) / 2)
            end
        end
...
Parse the output file from msolve and create a DataFrame of critical points.

# Arguments
- `file_path`: Path to the output file from msolve
...
"""
    parse_point(X::Vector{Vector{Vector{BigInt}}})::Vector{Float64}

Parse a nested vector of `BigInt` values and convert them into a vector of `Float64`.

# Arguments
- `X::Vector{Vector{Vector{BigInt}}}`: A nested vector where each element is a vector of two vectors, 
  each containing two `BigInt` values. The first value in each inner vector represents the numerator, 
  and the second value represents the exponent of the denominator (which is a power of 2).

# Returns
- Vector{Float64}: A vector of Float64 values, where each value is the average of the 
  two rational numbers represented by the input vectors.
...
# result is a vector of Float64 values: [1.0, 1.0]
```
"""
...
Calculate the average of a vector of numbers using rational arithmetic for precision.
...
"""
    safe_average(X::Vector{T}) where {T<:Number}

Calculate the average of a vector of numbers with safety checks for Inf/NaN results.
