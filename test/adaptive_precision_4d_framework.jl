...
function compare_critical_point_accuracy(func_name=:shubert; degree=8, samples=100)
    ...
    results[:accuracy_improvement] = max_error_f64 > 0 ? max_error_f64 / max_error_adaptive : 1.0
    ...
    if max_error_adaptive < max_error_f64 && max_error_f64 > 1e-15
        ...
        println("  ✅ AdaptivePrecision improvement: $(round(improvement, digits=2))x more accurate")
    elseif max_error_adaptive > max_error_f64 && max_error_adaptive > 1e-15
        degradation = max_error_adaptive / max_error_f64
        println("  ⚠️  AdaptivePrecision $(round(degradation, digits=2))x less accurate in this test")
    else
...
function optimize_4d_accuracy(func_name=:shubert; max_degree=12, max_samples=500)
    ...
    sample_sizes = [50, 100, 200, 300, 500][1:findfirst(x -> x > max_samples, [50, 100, 200, 300, 500, 1000])-1]
    ...
    println("  Best configuration: degree=$(best_config.degree), samples=$(best_config.samples)")
...
"""
function profile_4d_bottlenecks(func_name=:shubert; degree=8, samples=200, iterations=10)
    ...
    phases = Dict(
        "Test Input Creation" => () -> create_4d_test_input(func_name, STANDARD_CONFIG, samples=samples),
        "Float64 Construction" => () -> construct_4d_polynomial(TR, degree, Float64Precision),
        "Adaptive Construction" => () -> construct_4d_polynomial(TR, degree, AdaptivePrecision),
        "Monomial Conversion (F64)" => () -> begin
            pol = construct_4d_polynomial(TR, degree, Float64Precision)
            @polyvar x[1:4]
            to_exact_monomial_basis(pol, variables=x)
        end,
        "Monomial Conversion (Adaptive)" => () -> begin
    ...
    for (phase_name, phase_func) in phases
        ...
        if !isempty(times)
            ...
            println("  Mean time: $(round(mean_time, digits=4))s ± $(round(std_time, digits=4))s")
            println("  Range: $(round(min_time, digits=4))s - $(round(max_time, digits=4))s")
        else
    ...
    if !isempty(bottleneck_results)
        ...
        for (i, (phase, result)) in enumerate(sorted_phases)
            ...
            println("  $i. $phase: $(round(result[:mean], digits=4))s ($(round(percentage, digits=1))%)")
        end
...
"""
function optimize_4d_parameters(func_name=:shubert; target_accuracy=1e-6, max_time=60.0)
        ...
        for samples in sample_sizes
            ...
            try
                ...
                config_result = Dict(
                    ...
                    :feasible => (result[:adaptive_norm] <= target_accuracy) && (elapsed_time <= max_time)
                )
                ...
                println("  L2 norm: $(round(result[:adaptive_norm], digits=8)) $(result[:adaptive_norm] <= target_accuracy ? "✅" : "❌")")
                println("  Time: $(round(elapsed_time, digits=3))s $(elapsed_time <= max_time ? "✅" : "❌")")
    ...
    if !isempty(feasible_configs)
        ...
        for config in feasible_configs
            ...
            time_score = (config[:construction_time] - min_time) / (max_time_actual - min_time + 1e-10)
        ...
        println("   Degree=$(best_accuracy_config[:degree]), Samples=$(best_accuracy_config[:samples])")
        ...
        println("   Degree=$(balanced_config[:degree]), Samples=$(balanced_config[:samples])")
...
"""
function optimize_sparsity_thresholds(func_name=:sparse; degree=8, samples=200, max_accuracy_loss=0.01)
        ...
        for threshold in thresholds
            try
                if strategy_name == "Percentile"
                    ...
                    percentile_idx = max(1, round(Int, length(sorted_coeffs) * threshold / 100))
                ...
                # Apply truncation
                truncated_poly, stats = truncate_polynomial_adaptive(mono_adaptive, actual_threshold)
                ...
                println("  Threshold $threshold: $(round(sparsity_ratio*100, digits=1))% sparse, est. loss $(round(accuracy_loss*100, digits=3))% $(accuracy_loss <= max_accuracy_loss ? "✅" : "❌")")
    ...
    if !isempty(optimal_configs)
        ...
        best_sparsity_config = sort(optimal_configs, by=x -> x[:sparsity_ratio], rev=true)[1]

        # Find most conservative configuration (minimum accuracy loss)
        most_conservative_config = sort(optimal_configs, by=x -> x[:accuracy_loss_estimate])[1]
        ...
        println("   Sparsity: $(round(best_sparsity_config[:sparsity_ratio]*100, digits=1))%")
        println("   Est. accuracy loss: $(round(best_sparsity_config[:accuracy_loss_estimate]*100, digits=3))%")
        println("   Terms: $(best_sparsity_config[:kept_terms])/$(best_sparsity_config[:original_terms])")

        println("\n2. Most Conservative:")
        println("   Strategy: $(most_conservative_config[:strategy])")
        println("   Threshold: $(most_conservative_config[:threshold])")
        println("   Sparsity: $(round(most_conservative_config[:sparsity_ratio]*100, digits=1))%")
        println("   Est. accuracy loss: $(round(most_conservative_config[:accuracy_loss_estimate]*100, digits=3))%")
        println("   Terms: $(most_conservative_config[:kept_terms])/$(most_conservative_config[:original_terms])")
        ...
        for (i, config) in enumerate(pareto_configs[1:min(3, end)])
            println("  $i. $(config[:strategy]) $(config[:threshold]): $(round(config[:sparsity_ratio]*100, digits=1))% sparse, $(round(config[:accuracy_loss_estimate]*100, digits=3))% loss")
        end

        return DataFrame(results), best_sparsity_config, most_conservative_config, pareto_configs
...
"""
function analyze_degree_scaling(func_name=:shubert; degrees=[4,6,8,10,12], samples=200)
    ...
    for degree in degrees
        ...
        try
            ...
            result = Dict(
                ...
                :total_overhead => (adaptive_time + adaptive_conversion_time) / (f64_time + f64_conversion_time),
            ...
            println("  Construction: $(round(f64_time, digits=3))s → $(round(adaptive_time, digits=3))s ($(round(result[:construction_overhead], digits=2))x)")
            println("  Conversion: $(round(f64_conversion_time, digits=3))s → $(round(adaptive_conversion_time, digits=3))s ($(round(result[:conversion_overhead], digits=2))x)")
            println("  Total: $(round(result[:total_f64_time], digits=3))s → $(round(result[:total_adaptive_time], digits=3))s ($(round(result[:total_overhead], digits=2))x)")
            println("  Terms: $(result[:f64_terms]) → $(result[:adaptive_terms])")
            println("  L2 norm: $(round(result[:f64_norm], digits=8)) → $(round(result[:adaptive_norm], digits=8))")
    ...
    if !isempty(results)
        ...
        println("  Construction overhead: $(round(minimum(construction_overheads), digits=2))x - $(round(maximum(construction_overheads), digits=2))x")
        println("  Conversion overhead: $(round(minimum(conversion_overheads), digits=2))x - $(round(maximum(conversion_overheads), digits=2))x")
        println("  Total overhead: $(round(minimum(total_overheads), digits=2))x - $(round(maximum(total_overheads), digits=2))x")
...
"""
function analyze_sample_scaling(func_name=:shubert; degree=8, sample_sizes=[50,100,200,400,800])
    ...
    for samples in sample_sizes
        ...
        try
            ...
            result = Dict(
                ...
                :time_per_sample_adaptive => (adaptive_time + adaptive_conversion_time) / samples
            )
            ...
            println("  Construction: $(round(f64_time, digits=3))s → $(round(adaptive_time, digits=3))s ($(round(result[:construction_overhead], digits=2))x)")
            println("  Total: $(round(result[:total_f64_time], digits=3))s → $(round(result[:total_adaptive_time], digits=3))s ($(round(result[:total_overhead], digits=2))x)")
            println("  Time/sample: $(round(result[:time_per_sample_f64]*1000, digits=2))ms → $(round(result[:time_per_sample_adaptive]*1000, digits=2))ms")
            println("  L2 norm: $(round(result[:f64_norm], digits=8)) → $(round(result[:adaptive_norm], digits=8))")
    ...
    if !isempty(results)
        ...
        if length(results) >= 3
            ...
            println("    Float64: $(round(f64_scaling, digits=3))x ($(f64_scaling < 1.0 ? "improving" : "degrading"))")
            println("    Adaptive: $(round(adaptive_scaling, digits=3))x ($(adaptive_scaling < 1.0 ? "improving" : "degrading"))")
        end
        ...
        elseif max_norm_diff < 1e-10
            ...
            println("  ⚠️  Accuracy varies across sample sizes (max diff: $(max_norm_diff))")
        end
    end
...
"""
function analyze_coefficient_ranges(func_names=[:gaussian, :shubert, :sparse]; degree=8, samples=200)
        ...
        try
            ...
            if !isempty(f64_nonzero) && !isempty(adaptive_nonzero)
                f64_analysis = Dict(
                    ...
                    :log_dynamic_range => log10(maximum(f64_nonzero) / minimum(f64_nonzero))
                )
                ...
                adaptive_analysis = Dict(
                    ...
                    :dynamic_range => Float64(maximum(adaptive_nonzero) / minimum(adaptive_nonzero)),
                    :log_dynamic_range => log10(Float64(maximum(adaptive_nonzero) / minimum(adaptive_nonzero)))
                )
                ...
                result = Dict(
                    ...
                    :adaptive_log_dynamic_range => adaptive_analysis[:log_dynamic_range],
                    :dynamic_range_ratio => adaptive_analysis[:dynamic_range] / f64_analysis[:dynamic_range],
                    :precision_advantage => adaptive_analysis[:log_dynamic_range] - f64_analysis[:log_dynamic_range],
                    :f64_norm => pol_f64.nrm,
                    :adaptive_norm => pol_adaptive.nrm,
                    :norm_difference => abs(pol_adaptive.nrm - pol_f64.nrm)
                )
                ...
                println("  Float64 coefficient range: [$(scientific_notation(f64_analysis[:min])), $(scientific_notation(f64_analysis[:max]))]")
                println("  Adaptive coefficient range: [$(scientific_notation(adaptive_analysis[:min])), $(scientific_notation(adaptive_analysis[:max]))]")
                println("  Dynamic range: F64=$(round(f64_analysis[:log_dynamic_range], digits=1)) decades, Adaptive=$(round(adaptive_analysis[:log_dynamic_range], digits=1)) decades")
                println("  Precision advantage: $(round(result[:precision_advantage], digits=1)) decades")
                println("  L2 norms: F64=$(round(result[:f64_norm], digits=8)), Adaptive=$(round(result[:adaptive_norm], digits=8))")
    ...
    if !isempty(results)
        ...
        println("  Precision advantage: $(round(minimum(precision_advantages), digits=1)) - $(round(maximum(precision_advantages), digits=1)) decades")
        println("  Dynamic range improvement: $(round(minimum(dynamic_range_ratios), digits=2))x - $(round(maximum(dynamic_range_ratios), digits=2))x")
        ...
        println("  Best precision advantage: $best_function ($(round(best_advantage, digits=1)) decades)")
        ...
        if !isempty(significant_benefit)
            println("  Functions with significant benefit (>2 decades): $(length(significant_benefit))")
            for r in significant_benefit
                println("    $(r[:function]): $(round(r[:precision_advantage], digits=1)) decades")
            end
        else
        ...
        elseif max_norm_diff < 1e-10
            ...
            println("  ⚠️  Accuracy varies across coefficient ranges (max diff: $(max_norm_diff))")
        end
    end
...
"""
function cache_function_evaluations!(cache::FunctionEvaluationCache, func_name::String, points::Vector{Vector{Float64}}, func::Function)
...
"""
function get_cached_evaluation(cache::FunctionEvaluationCache, func_name::String, point::Vector{Float64}, func::Function)
...
"""
function generate_4d_grid_points(center::Vector{Float64}, range::Float64, samples_per_dim::Int)
...
"""
function benchmark_cached_construction(func_name=:shubert; degree=8, samples=200, cache_samples_per_dim=10)
...
"""
function generate_grid_points(strategy::UniformGrid, center::Vector{Float64}, range::Float64, func::Function)
    return generate_4d_grid_points(center, range, strategy.samples_per_dim)
end

function generate_grid_points(strategy::RandomGrid, center::Vector{Float64}, range::Float64, func::Function)
...
function generate_grid_points(strategy::AdaptiveGrid, center::Vector{Float64}, range::Float64, func::Function)
...
function compare_grid_strategies(func_name=:shubert; degree=8, strategies=nothing)
    ...
    for (strategy_name, strategy) in strategies
        ...
        try
            ...
            cache = FunctionEvaluationCache()
            start_time = time()
            cached_count = cache_function_evaluations!(cache, string(func_name), grid_points, func)
            cache_time = time() - start_time
            ...
            result = Dict(
                ...
                :points_per_second => length(grid_points) / (grid_generation_time + cache_time)
            )
    ...
    if !isempty(results)
        ...
        println("  Most efficient: $(sorted_by_efficiency[1][:strategy]) ($(round(sorted_by_efficiency[1][:cache_efficiency]*100, digits=1))%)")
        println("  Fastest setup: $(sorted_by_speed[1][:strategy]) ($(round(sorted_by_speed[1][:points_per_second], digits=1)) pts/s)")
        println("  Best coverage: $(sorted_by_coverage[1][:strategy]) ($(sorted_by_coverage[1][:grid_points]) points)")
        ...
        for result in results
            ...
            balanced_score = 0.4 * efficiency_score + 0.3 * speed_score + 0.3 * coverage_score
        ...
        if any(r[:grid_points] > 1000 for r in results)
            println("  For large-scale problems: Consider uniform grids for predictable performance")
        end
...
struct PolynomialStorage
    ...
    function PolynomialStorage(degree::Int, variables::Vector{Symbol}=[:x1, :x2, :x3, :x4];
...
function store_polynomial!(storage::PolynomialStorage, polynomial)
    ...
    for term in terms(polynomial)
        ...
        try
            ...
            exponent_key = [term_hash % 10, (term_hash ÷ 10) % 10, (term_hash ÷ 100) % 10, (term_hash ÷ 1000) % 10]
...
"""
function analyze_storage_efficiency(func_names=[:gaussian, :shubert, :sparse]; degree=8, samples=200)
    ...
    for func_name in func_names
        ...
        try
            ...
            for threshold in thresholds
                storage = PolynomialStorage(degree, [:x1, :x2, :x3, :x4], sparsity_threshold=threshold)
                ...
                println("  Threshold $(threshold): $(terms_stored) terms, $(round(result[:estimated_memory_mb], digits=2)) MB")
            end
    ...
    if !isempty(results)
        ...
        println("  Memory savings: $(round(minimum(memory_savings)*100, digits=1))% - $(round(maximum(memory_savings)*100, digits=1))%")
        println("  Sparsity ratios: $(round(minimum(sparsity_ratios)*100, digits=1))% - $(round(maximum(sparsity_ratios)*100, digits=1))%")

        # Best configurations
        best_savings_idx = argmax(memory_savings)
        best_config = results[best_savings_idx]

        println("  Best memory savings: $(best_config[:function]) with threshold $(best_config[:threshold])")
        println("    Savings: $(round(best_config[:memory_savings]*100, digits=1))%")
        println("    Memory: $(round(best_config[:estimated_memory_mb], digits=2)) MB vs $(round(best_config[:dense_memory_mb], digits=2)) MB dense")
        ...
        for (func, func_results) in function_groups
            best_func_result = sort(func_results, by=x -> x[:memory_savings], rev=true)[1]
            println("  $func: Use threshold $(best_func_result[:threshold]) for $(round(best_func_result[:memory_savings]*100, digits=1))% savings")
        end
        ...
        println("  Theoretical max terms: deg 4→$(degree_4_terms), deg 8→$(degree_8_terms), deg 12→$(degree_12_terms)")
        println("  Memory scaling (dense): deg 4→$(round(degree_4_terms*80/(1024^2), digits=2))MB, deg 8→$(round(degree_8_terms*80/(1024^2), digits=2))MB, deg 12→$(round(degree_12_terms*80/(1024^2), digits=2))MB")

        if maximum(sparsity_ratios) > 0.8
            println("  ✅ High sparsity enables significant memory savings for large degrees")
        else
