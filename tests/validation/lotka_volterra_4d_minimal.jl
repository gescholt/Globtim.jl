...
# Summary
println("Quality: $(pol_cheb.nrm < 1e-3 ? "Excellent" : pol_cheb.nrm < 0.1 ? "Good" : "Poor"), Critical points: $(nrow(df)), Minimizers: $(nrow(df_min))")

if nrow(df_min) > 0
    best_min = df_min[argmin(df_min.value), :]
    println("Best minimum: [$(round(best_min.x1, digits=3)), $(round(best_min.x2, digits=3)), $(round(best_min.x3, digits=3)), $(round(best_min.x4, digits=3))] = $(round(best_min.value, digits=6))")
end