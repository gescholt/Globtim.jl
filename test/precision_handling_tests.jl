...
@testset "Precision Handling Pipeline Tests" begin
    ...
    @testset "Coefficient Vector Conversion Tests" begin
        ...
        @testset "Mixed Coefficient Types" begin
            ...
            converted_f64 = [Globtim._convert_value(c, Float64Precision) for c in mixed_coeffs]
            ...
            @test_nowarn converted_rational = [Globtim._convert_value(c, RationalPrecision) for c in mixed_coeffs]
            converted_rational = [Globtim._convert_value(c, RationalPrecision) for c in mixed_coeffs]
            @test all(c isa Rational for c in converted_rational)
            
            # AdaptivePrecision should use BigFloat
            converted_adaptive = [Globtim._convert_value(c, AdaptivePrecision) for c in mixed_coeffs]
        ...
        @testset "Zero-Heavy Coefficient Vectors" begin
            ...
            for precision_type in [Float64Precision, RationalPrecision, BigFloatPrecision, AdaptivePrecision]
                @test_nowarn converted = [Globtim._convert_value(c, precision_type) for c in zero_heavy_coeffs]
                
                converted = [Globtim._convert_value(c, precision_type) for c in zero_heavy_coeffs]
        ...
        @testset "Constructor → Polynomial System Precision Consistency" begin
            ...
            for i in 1:samples_per_dim
                for j in 1:samples_per_dim
                    θ = [
                        center[1] + sample_range * (2*(i-1)/(samples_per_dim-1) - 1),
                        center[2] + sample_range * (2*(j-1)/(samples_per_dim-1) - 1)
                    ]
        ...
        @testset "Condition Number Behavior Across Precisions" begin
            ...
            for coeffs in [well_conditioned, ill_conditioned]
                for precision_type in [Float64Precision, AdaptivePrecision]
                    @test_nowarn begin
                        pol = Globtim.construct_orthopoly_polynomial(
                            [z], coeffs, (:one_d_for_all, length(coeffs)-1), :chebyshev, precision_type;
                            verbose = false
                        )
                    end
                end
    ...
    @testset "Performance and Memory Tests" begin
        ...
        @testset "Precision Conversion Performance" begin
            ...
            @test (@elapsed [Globtim._convert_value(c, Float64Precision) for c in large_coeffs]) < 1.0
            
            # Time other precision conversions (should complete in reasonable time)
            @test (@elapsed [Globtim._convert_value(c, AdaptivePrecision) for c in large_coeffs]) < 5.0
            @test (@elapsed [Globtim._convert_value(c, BigFloatPrecision) for c in large_coeffs]) < 5.0
        end
