#!/usr/bin/env julia
# Constrained Lotka-Volterra 4D Experiment: LOG L2 Loss Function
# Usage: julia --project=. run_constrained_lv_log_loss.jl --GN 16 --deg-min 4 --deg-max 8 --domain 0.3 --seed 42
...
println("="^80)
println("Constrained Lotka-Volterra 4D - LOG L2 Loss Function")
println("="^80)
println("  Loss function: LOG (log₂ of L2 norm)")
...
# Create error function with LOG L2 norm
error_func = make_error_distance(
    ...
    TIME_INTERVAL,
    NUM_POINTS,
    log_L2_norm  # LOG loss function
)

println("\nError function created (LOG L2 norm)")

# Test error function
test_error = error_func(P_CENTER)
println("Log error at p_center: ", test_error)
...
open(joinpath(results_dir, "summary.txt"), "w") do io
    println(io, "Constrained Lotka-Volterra 4D - LOG L2 Loss Experiment Summary")
    println(io, "="^80)
    println(io, "Generated: $(now())")
    println(io, "")
    println(io, "Configuration:")
    println(io, "  Loss function: LOG (log₂ of L2 norm)")
