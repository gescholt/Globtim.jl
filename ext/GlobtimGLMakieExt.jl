...
function Globtim.plot_all_eigenvalues(f::Function, df::DataFrames.DataFrame; sort_by=:magnitude)
    ...
    for (subplot_idx, ptype) in enumerate(available_types)
        ...
        ax = GLMakie.Axis(fig[subplot_idx, 1], 
        ...
        for (plot_idx, orig_idx) in enumerate(sorted_indices)
            ...
            GLMakie.lines!(ax, fill(x_pos, length(plot_vals)), plot_vals, 
            ...
            for (eig_idx, (eigenval, plot_val)) in enumerate(zip(eigenvals, plot_vals))
                GLMakie.scatter!(ax, [x_pos], [plot_val], 
    ...
    eigenval_legend_elements = [GLMakie.MarkerElement(color=eigenval_colors[i], marker=:circle, markersize=10) 
    ...
    type_legend_elements = [GLMakie.MarkerElement(color=:black, marker=:circle, markersize=10, strokecolor=type_colors[ptype], strokewidth=2) 
    ...
    GLMakie.Legend(fig[1:n_types, 2], eigenval_legend_elements, eigenval_legend_labels, 
                     "Eigenvalue Order", tellheight=false, framevisible=true)
    GLMakie.Legend(fig[1:n_types, 3], type_legend_elements, type_legend_labels, 
...
function Globtim.plot_raw_vs_refined_eigenvalues(f::Function, df_raw::DataFrames.DataFrame, df_refined::DataFrames.DataFrame; sort_by=:euclidean_distance)
    ...
    for (subplot_idx, ptype) in enumerate(available_types)
        ...
        for (pair_idx, (raw_idx, refined_idx, distance)) in enumerate(type_matches)
            ...
            for (eig_idx, eigenval) in enumerate(raw_sorted[1:n_eigenvals])
                ...
                GLMakie.scatter!(ax, [x_pos], [y_pos], 
                          color=eigenval_colors[min(eig_idx, length(eigenval_colors))],
                          marker=:circle,
                          markersize=8,
                          strokecolor=type_color,
            ...
            for eig_idx in 1:n_eigenvals
                ...
                GLMakie.lines!(ax, [x_pos, x_pos], [raw_y, refined_y],
                         color=eigenval_colors[min(eig_idx, length(eigenval_colors))],
                         linestyle=:solid,
                         linewidth=1.5,
                         alpha=0.7)
            ...
            if distance > 0
                GLMakie.text!(ax, x_pos, minimum(refined_sorted) + refined_y_offset - 0.1,
        ...
        GLMakie.hlines!(ax, [0], color=type_color, linestyle=:solid, alpha=0.3, linewidth=2)
    end
    ...
    raw_refined_elements = [
        GLMakie.MarkerElement(color=:blue, marker=:circle, markersize=10, alpha=0.6),
        GLMakie.MarkerElement(color=:blue, marker=:circle, markersize=10, alpha=1.0)
    ]
    ...
    GLMakie.Legend(fig[1:n_types, 2], eigenval_legend_elements, eigenval_legend_labels, 
                     "Eigenvalue Order", tellheight=false, framevisible=true)
    GLMakie.Legend(fig[1:n_types, 3], raw_refined_elements, raw_refined_labels, 
