...
function analyze_degree_progression(experiments::Vector{ExperimentData})
    ...
    valid_exps =
        filter(exp -> exp.l2_norm !== nothing && exp.degree !== nothing, experiments)
    ...
    return Dict(
        ...
        "total_improvement" =>
            length(improvements) > 0 ? (l2_norms[1] - l2_norms[end]) / l2_norms[1] * 100.0 :
            0.0,
...
function create_computational_summary(data::ExperimentData)
    ...
    if haskey(sampling_metrics, "theoretical_monomials")
        ...
        if haskey(sampling_metrics, "sample_monomial_ratio")
            println(
                io,
                "Sample_ratio: $(@sprintf("%.3f", sampling_metrics["sample_monomial_ratio"]))"
            )
