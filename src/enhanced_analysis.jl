        ...
        if length(stats_objects) > 1
            ...
            if export_tables
                ...
                try
                    ...
                    @warn "Failed to export comparative table: $export_filename" exception=e
                end
            end
        end
...
function create_statistical_summary(df_enhanced::DataFrame)
    ...
    # Count by type
    type_counts = DataFrames.combine(DataFrames.groupby(df_enhanced, :critical_point_type), DataFrames.nrow => :count)
    ...
    push!(lines, "├─────────────────┼───────────┼───────────┤")
    push!(lines, "│ $(rpad("TOTAL", 15)) │ $(lpad(total_points, 9)) │ $(lpad("100.0%", 9)) │")
    push!(lines, "└─────────────────┴───────────┴───────────┘")
...
function quick_table_preview(f, df, TR; point_types=[:minimum, :maximum])
    ...
    if !hasproperty(df, :critical_point_type) || !hasproperty(df, :hessian_norm)
        ...
        df_enhanced, _ = analyze_critical_points(f, df, TR, enable_hessian=true, verbose=false)
    else
        df_enhanced = df
    end
    ...
    for point_type in point_types
        ...
        if stats_table.hessian_stats.count > 0
            ...
            println("  Hessian norm: $(round(hs.mean, digits=3)) ± $(round(hs.std, digits=3))")
            if ca.total_count > 0
