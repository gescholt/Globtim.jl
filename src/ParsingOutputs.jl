...
# function parse_rational_interval(interval_str::String)
#     # Extract numerator/denominator with regex
#     pattern = r"(\d+)\s*/\s*2\^(\d+)"
#     matches = collect(eachmatch(pattern, interval_str))
...
#     points = map(matches) do m
#         num = parse(BigInt, m[1])
#         den_exp = parse(Int, m[2])
#         Float64(num / BigFloat(2)^den_exp)
#     end
