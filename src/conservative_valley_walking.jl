...
```
"""
function conservative_valley_walk(f, start_point::Vector{Float64}, direction::Vector{Float64}, 
                                config::ConservativeValleyConfig = ConservativeValleyConfig())
    ...
    if norm(start_grad) > config.gradient_tolerance
        return steps, "Starting point is not a critical point (gradient too large: $(norm(start_grad)))"
    end
    
    if start_min_eigenval > config.eigenvalue_threshold
        return steps, "Starting point is not in a valley (no small eigenvalues: $(start_min_eigenval))"
    end
    
    # Add starting point to steps
    push!(steps, ConservativeValleyStep(
    ...
    for step_num in 1:config.max_steps
        ...
        if !function_valid
            ...
            if current_step_size < config.min_step_size
                return steps, "Step size became too small: $(current_step_size) < $(config.min_step_size)"
            end
        ...
        if norm(candidate_grad) > config.gradient_tolerance
            return steps, "Left critical point region (gradient norm: $(norm(candidate_grad)))"
        end
        ...
        if f_change < -config.function_tolerance
            current_step_size = min(current_step_size * config.step_increase_factor, config.max_step_size)
        end
    end
...
function explore_valley_manifold_conservative(f, seed_point::Vector{Float64}, 
    ...
    total_accepted = sum(sum(step.step_accepted for step in steps) for steps in values(all_steps))
    ...
    summary = (
        ...
        min_function_value = length(all_function_values) > 0 ? minimum(all_function_values) : metrics.function_value,
        max_function_value = length(all_function_values) > 0 ? maximum(all_function_values) : metrics.function_value,
        function_value_range = length(all_function_values) > 0 ? maximum(all_function_values) - minimum(all_function_values) : 0.0,
