...
@testset "thread_evals kwarg" begin
    ...
    @testset "estimate_subdomain_error (threaded == sequential)" begin
        ...
        l2_seq = Globtim.estimate_subdomain_error(sphere, tree_seq.subdomains[1], 4;
                                                  basis=:chebyshev,
                                                  thread_evals=false)
        l2_par = Globtim.estimate_subdomain_error(sphere, tree_par.subdomains[1], 4;
                                                  basis=:chebyshev,
                                                  thread_evals=true)
    ...
    @testset "adaptive_refine produces identical trees" begin
        ...
        tree_seq = Globtim.adaptive_refine(sphere, bounds, 4;
                                           l2_tolerance=1e-3,
                                           ...
                                           verbose=false,
                                           thread_evals=false)
        tree_par = Globtim.adaptive_refine(sphere, bounds, 4;
                                           l2_tolerance=1e-3,
                                           tolerance_mode=:absolute,
                                           max_depth=3,
                                           max_leaves=50,
        ...
        seq_errors = sort([tree_seq.subdomains[id].l2_error for id in keys(tree_seq.subdomains)])
        par_errors = sort([tree_par.subdomains[id].l2_error for id in keys(tree_par.subdomains)])
    ...
    @testset "two_phase_refine plumbs thread_evals" begin
        ...
        tree_seq = Globtim.two_phase_refine(sphere, bounds, 4;
                                            ...
                                            verbose=false,
                                            thread_evals=false)
        tree_par = Globtim.two_phase_refine(sphere, bounds, 4;
                                            coarse_tolerance=1e-2,
                                            fine_tolerance=1e-3,
                                            tolerance_mode=:absolute,
                                            max_depth=2,
    ...
    @testset "default (thread_evals=false) is unchanged" begin
        ...
        l2 = Globtim.estimate_subdomain_error(sphere, tree.subdomains[1], 4; basis=:chebyshev)
        @test isfinite(l2)
