...
"""
function create_orthant_test_inputs(f::Function, base_center::Vector{Float64}, 
                                   base_range::Float64, tolerance::Float64;
                                   overlap_factor::Float64=0.1)
    ...
    for (i, center) in enumerate(orthant_centers)
        TR = test_input(f, dim=4, center=center, sample_range=subdomain_range, tolerance=tolerance)
        push!(test_inputs, TR)
    end
...
"""
function filter_points_by_orthant(df::DataFrame, orthant_id::Int, base_center::Vector{Float64})
    mask = Bool[]
...
"""
function merge_orthant_results(orthant_dfs::Vector{DataFrame}, base_center::Vector{Float64};
                              distance_tolerance::Float64=1e-6)
