#!/usr/bin/env julia
# Lotka-Volterra 4D Parameterized Experiment Runner
# Usage: julia --project=. run_lv4d_experiment.jl --GN 10 --deg-min 4 --deg-max 6 --domain 0.1 [--local]
...
# Include SimpleOutputOrganizer for experiment directory management
...
# Create experiment directory using SimpleOutputOrganizer
exp_config = Dict{String, Any}(
    "objective_name" => "lotka_volterra_4d",
    "campaign" => IS_LOCAL ? "lv4d_local_test" : "lv4d_hpc_experiment",
    "dimension" => 4,
    "GN" => GN,
...
experiment_id = "lv4d_GN$(GN)_deg$(DEGREE_MIN)-$(DEGREE_MAX)_domain$(DOMAIN_RANGE)"
results_dir = create_experiment_dir(exp_config; experiment_id=experiment_id)

println("\n" * "="^80)
println("LOTKA-VOLTERRA 4D PARAMETERIZED EXPERIMENT")
println(IS_LOCAL ? " (LOCAL RUN)" : " (HPC RUN)")
println("="^80)
...
# Note: experiment_config.json already saved by create_experiment_dir()
...
open(joinpath(results_dir, "summary.txt"), "w") do io
    println(io, "Lotka-Volterra 4D Experiment Summary")
    println(io, IS_LOCAL ? "(LOCAL RUN)" : "(HPC RUN)")
    println(io, "="^60)
