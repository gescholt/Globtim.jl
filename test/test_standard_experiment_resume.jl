...
function _fake_cfg(;
    ...
    basis = :chebyshev,
    domain_size = 0.1,
    truncation_threshold = nothing,
    truncation_mode = :relative,
)
...
function _success_result(degree::Int)
    return _DR(
        ...
        [0.0],
        0.0,
        nothing,                # best_estimate, best_objective, recovery_error
        1e-6,
        1e-9,
        1.0,                    # l2_approx_error, relative_l2_error, condition_number
        10,
        10,                             # n_total_coeffs, support_size
        nothing,
        nothing,
        nothing,          # truncation_*
        0.1,
