...
Globtim ships as a monorepo of Julia workspace packages (not yet in the General
registry). Clone it and instantiate the `dev` profile, which carries all three
packages plus the plotting backends:

```bash
git clone https://github.com/gescholt/globopt_merged.git && cd globopt_merged
julia --project=profiles/dev -e 'using Pkg; Pkg.instantiate(workspace=true)'
```

From then on, launch Julia with `--project=profiles/dev` and `using Globtim`,
`using GlobtimPostProcessing`, `using GlobtimPlots` all resolve.
...
[`examples/custom_function_demo.jl`](https://github.com/gescholt/globopt_merged/blob/main/pkg/globtim/examples/custom_function_demo.jl)):
...
[GlobtimPostProcessing docs](https://github.com/gescholt/globopt_merged/tree/main/pkg/globtimpostprocessing).
...
is regenerated by [`pkg/globtimplots/examples/gallery.jl`](https://github.com/gescholt/globopt_merged/blob/main/pkg/globtimplots/examples/gallery.jl).)*

`GlobtimPlots` also draws Morse spectra, adaptive-subdivision partitions, and
degree-convergence plots — see [GlobtimPlots](globtimplots.md) and its
[API reference](https://github.com/gescholt/globopt_merged/tree/main/pkg/globtimplots).

## Where to go next

- [Getting Started](getting_started.md) — the globtim workflow in more detail
- [Examples](examples.md) — the eight runnable demo scripts shipped in `pkg/globtim/examples/`
