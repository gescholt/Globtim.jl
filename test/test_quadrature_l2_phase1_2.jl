...
@testset "Quadrature L2 Norm - Phase 1 & 2 Tests" begin
    ...
    @testset "Phase 1: Core Functionality" begin
        ...
        @testset "1.1 Polynomial Exactness (1D)" begin
            ...
            f_const = x -> 1.0
            @test compute_l2_norm_quadrature(f_const, [5], :chebyshev) ≈ sqrt(2.0) rtol=1e-12
            @test compute_l2_norm_quadrature(f_const, [10], :chebyshev) ≈ sqrt(2.0) rtol=1e-12
            
            # Test quadratic: f(x) = x²
            # Analytical L2 norm: sqrt(∫₋₁¹ x⁴ dx) = sqrt(2/5)
            f_quad = x -> x[1]^2
            @test compute_l2_norm_quadrature(f_quad, [3], :chebyshev) ≈ sqrt(2/5) rtol=1e-12
            @test compute_l2_norm_quadrature(f_quad, [5], :chebyshev) ≈ sqrt(2/5) rtol=1e-12
            
            # Test quartic: f(x) = x⁴
            # Analytical L2 norm: sqrt(∫₋₁¹ x⁸ dx) = sqrt(2/9)
            f_quartic = x -> x[1]^4
            @test compute_l2_norm_quadrature(f_quartic, [5], :chebyshev) ≈ sqrt(2/9) rtol=1e-12
            @test compute_l2_norm_quadrature(f_quartic, [10], :chebyshev) ≈ sqrt(2/9) rtol=1e-12
            
            # Test linear (odd function): f(x) = x
            # Analytical L2 norm: sqrt(∫₋₁¹ x² dx) = sqrt(2/3)
            f_linear = x -> x[1]
            @test compute_l2_norm_quadrature(f_linear, [5], :chebyshev) ≈ sqrt(2/3) rtol=1e-12
        end
        ...
        @testset "1.2 Basic Multi-dimensional (2D only)" begin
            ...
            f_const_2d = x -> 1.0
            @test compute_l2_norm_quadrature(f_const_2d, [5, 5], :chebyshev) ≈ 2.0 rtol=1e-12
            @test compute_l2_norm_quadrature(f_const_2d, [10, 10], :chebyshev) ≈ 2.0 rtol=1e-12
            ...
            @test compute_l2_norm_quadrature(f_sep, [10, 10], :chebyshev) ≈ 2/5 rtol=1e-12
        ...
        @testset "1.3 Single Basis Type (Chebyshev only)" begin
            ...
            @test compute_l2_norm_quadrature(f_high_deg, [7], :chebyshev) ≈ sqrt(2/13) rtol=1e-12
            @test compute_l2_norm_quadrature(f_high_deg, [10], :chebyshev) ≈ sqrt(2/13) rtol=1e-12
        end
        ...
        @testset "1.4 Basic Convergence (1D and 2D)" begin
            ...
            @test begin
                norms_1d = [compute_l2_norm_quadrature(f_exp_1d, [n], :chebyshev) for n in grid_sizes_1d]
                # Check that all norms are positive
                all(n -> n > 0, norms_1d) &&
                # Check convergence: differences should decrease
                begin
                    diffs = [abs(norms_1d[i+1] - norms_1d[i]) for i in 1:length(norms_1d)-1]
                    # Either decreasing or very small
                    all(i -> diffs[i+1] < diffs[i] || diffs[i+1] < 1e-10, 1:length(diffs)-1)
                end
            end
            ...
            @test begin
                norms_2d = [compute_l2_norm_quadrature(f_exp_2d, [n, n], :chebyshev) for n in grid_sizes_2d]
                # Check that all norms are positive
                all(n -> n > 0, norms_2d) &&
                # Check convergence
                begin
                    diffs = [abs(norms_2d[i+1] - norms_2d[i]) for i in 1:length(norms_2d)-1]
    ...
    @testset "Phase 2: Extended Dimensions" begin
        ...
        @testset "2.1 3D and 4D Integration" begin
            ...
            @test compute_l2_norm_quadrature(f_const, [5, 5, 5], :chebyshev) ≈ 2*sqrt(2) rtol=1e-12
            
            # 4D: L2 norm = sqrt(16) = 4
            @test compute_l2_norm_quadrature(f_const, [5, 5, 5, 5], :chebyshev) ≈ 4.0 rtol=1e-12
