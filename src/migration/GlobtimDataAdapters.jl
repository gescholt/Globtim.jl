...
get_domain_center(a::ApproxPolyAdapter) =
    length(a.wrapped.grid) > 0 ? vec(mean(a.wrapped.grid, dims = 1)) : Float64[]
...
get_function_values(c::CriticalPointDataFrameAdapter) =
    haskey(c.wrapped, :z) ? c.wrapped.z : Float64[]
get_point_types(c::CriticalPointDataFrameAdapter) =
    haskey(c.wrapped, :critical_point_type) ? c.wrapped.critical_point_type : Symbol[]
get_convergence_info(c::CriticalPointDataFrameAdapter) =
    haskey(c.wrapped, :convergence_info) ? c.wrapped.convergence_info : Dict[]
...
function test_adapters(pol::ApproxPoly, TR::test_input, df::DataFrame)
    ...
    println(
        "  Has function values: $(length(get_function_values(adapters.critical_points)) > 0)"
    )
