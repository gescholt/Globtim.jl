...
df_cheb, df_min_cheb = analyze_critical_points(f, df_cheb, TR, tol_dist = 0.325)
sort!(df_cheb, :close, rev = true)

fig5 = plot_polyapprox_levelset(
    pol_cheb,
    TR,
    df_cheb,
    df_min_cheb,
...
df_lege, df_min_lege = analyze_critical_points(f, df_lege, TR, tol_dist = 0.325)
sort!(df_lege, :z, rev = true)
fig6 = plot_polyapprox_levelset(
    pol_lege,
    TR,
    df_lege,
    df_min_lege,
