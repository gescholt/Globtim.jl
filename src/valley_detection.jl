...
```
"""
function detect_valley_at_point(
    f,
    x::Vector{Float64},
    config::ValleyDetectionConfig = ValleyDetectionConfig()
)
    ...
    if valley_dimension > 0
        ...
        if length(non_zero_eigenvalues) > 0
            eigenvalue_gap =
                minimum(abs.(non_zero_eigenvalues)) /
                maximum(abs.(sorted_eigenvalues[near_zero_mask]))
...
```
"""
function follow_valley_manifold(
    f,
    ...
    for step in 1:(config.max_manifold_steps)
        ...
        if !new_valley_info.is_valley ||
           new_valley_info.valley_dimension != valley_info.valley_dimension
...
"""
function project_to_critical_manifold(
    f,
    x::Vector{Float64},
...
"""
function analyze_valleys_in_critical_points(
    f,
    df::DataFrame,
    config::ValleyDetectionConfig = ValleyDetectionConfig()
)
    ...
    for i in 1:n_points
        ...
        df_enhanced[i, :smallest_eigenvalue] =
            length(valley_info.eigenvalues) > 0 ?
            minimum(abs.(valley_info.eigenvalues)) : 0.0

        # Compute eigenvalue gap
        if valley_info.is_valley &&
           length(valley_info.eigenvalues) > valley_info.valley_dimension
            zero_eigenvals = abs.(valley_info.eigenvalues[1:(valley_info.valley_dimension)])
            nonzero_eigenvals =
                abs.(valley_info.eigenvalues[(valley_info.valley_dimension + 1):end])
            if length(nonzero_eigenvals) > 0
                df_enhanced[i, :eigenvalue_gap] =
                    minimum(nonzero_eigenvals) / maximum(zero_eigenvals)
            end
        end
    end
