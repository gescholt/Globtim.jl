"""
Simple HPC Test Creation

Creates a minimal benchmark test without requiring all packages to be installed.
This generates SLURM scripts that can be submitted to test the infrastructure.
"""
...
#SBATCH --mem=8G
#SBATCH --output=$job_dir/slurm_output/globtim_test_$job_id_%j.out
#SBATCH --error=$job_dir/slurm_output/globtim_test_$job_id_%j.err
...
try
    ...
    if nrow(results.minima) > 0
        ...
        # Save as simple text file (no JSON3 needed)
...
info_content = """# Simple Globtim Test Experiment
...
## Test Description
- **Function:** 4D Sphere (global minimum at origin)
- **Degree:** 4
- **Samples:** 100
- **Expected:** Should find minimizers very close to [0,0,0,0]

## Files Created
- `$job_dir/test_job.sh` - SLURM job script
- `$helper_path` - Management script
...
## Expected Results
- **L2 error:** < 1e-10 (should be very small for Sphere function)
- **Minimizers:** Should find points very close to origin
- **Distance to global:** < 0.01 for good convergence
- **Convergence rate:** > 80% of minimizers within 0.1 of origin
"""

open("$base_dir/README.md", "w") do io
    write(io, info_content)
end

println("✓ Created experiment documentation: $base_dir/README.md")
println()
