...
try
    ...
    key_precision_exports =
        [:Float64Precision, :RationalPrecision, :AdaptivePrecision, :BigFloatPrecision]
    ...
    if :construct_orthopoly_polynomial in available_functions &&
       :solve_polynomial_system in available_functions
    ...
    if all(
        func in available_functions for
        func in [:_convert_value, :construct_orthopoly_polynomial, :solve_polynomial_system]
    )
    ...
    if any(
        func ∉ available_functions for
        func in [:construct_orthopoly_polynomial, :solve_polynomial_system]
    )
