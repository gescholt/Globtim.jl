# ================================================================================
# Enhanced Degree Convergence Analysis V3 - With Per-Subdomain Distance Tracking
# ================================================================================
# 
# Extends V2 with:
# 1. Per-subdomain distance tracking for minimizers
# 2. Individual subdomain traces on distance plot (like L2-norm plot)
# 3. Modular design with separate tracking and visualization modules
#
# ================================================================================
...
include("../src/MinimizerTracking.jl")
include("../src/EnhancedVisualization.jl")
...
using .MinimizerTracking
using .EnhancedVisualization
...
# Constants
const TRESH = 0.1  # Default threshold for minimizer recovery
...
function compute_enhanced_distance_stats(computed_points::Vector{Vector{Float64}}, 
    ...
    distances = [minimum(norm(cp - tm) for cp in computed_points) for tm in true_minimizers]
...
function run_enhanced_analysis_v2(degrees::Vector{Int}, gn::Int;
    ...
    true_minimizers = load_true_minimizers(joinpath(@__DIR__, "../data/4d_min_min_domain.csv"))
    
    # Setup output directory
    if output_dir === nothing
        timestamp = Dates.format(now(), "HH-MM")
        output_dir = joinpath(@__DIR__, "../outputs", "enhanced_v3_$(timestamp)")
    end
    mkpath(output_dir)
    ...
    # Assign minimizers to subdomains
    minimizer_assignment = assign_minimizers_to_subdomains(true_minimizers, subdomains)
    ...
    for degree in degrees
        ...
        # Compute subdomain-specific distance data
        subdomain_distances_for_degree = Dict{String, SubdomainDistanceData}()
        
        for subdomain in subdomains
            subdomain_points = computed_by_subdomain[subdomain.label]
            minimizer_indices = minimizer_assignment[subdomain.label]
            
            distances = compute_subdomain_distances(subdomain_points, true_minimizers, minimizer_indices)
            
            subdomain_distances_for_degree[subdomain.label] = SubdomainDistanceData(
                subdomain.label,
                degree,
                distances,
                !isempty(minimizer_indices),
                length(minimizer_indices)
            )
        end
        
        subdomain_distance_data[degree] = subdomain_distances_for_degree
    ...
    create_enhanced_plots_v3(summary_df, distance_data, global_distance_data, 
                            l2_data_by_degree, global_l2_by_degree, 
                            subdomain_distance_data, output_dir)
...
# Distance plot function removed - now using plot_distance_with_subdomains from EnhancedVisualization module
...
"""
function create_enhanced_plots_v3(summary_df::DataFrame,
                                distance_data::Dict{Int, EnhancedDistanceStats},
                                global_data::Dict{Int, EnhancedDistanceStats},
                                l2_data_by_degree::Dict,
                                global_l2_by_degree::Dict,
                                subdomain_distance_data::Dict,
                                output_dir::String)
    # Create L2 plot (unchanged)
    create_enhanced_l2_plot(summary_df, l2_data_by_degree, global_l2_by_degree, output_dir)
    
    # Create enhanced distance plot with subdomain traces
    plot_distance_with_subdomains(distance_data, global_data, subdomain_distance_data, 
                                 output_dir, threshold=TRESH)
