...
@testset "Anisotropic Grid Integration Tests" begin
    ...
    @testset "Performance Comparison" begin
        ...
        for (grid_size, name) in configs
            ...
            # Time approximation
            t = @elapsed pol = Globtim.MainGenerate(f, n, grid_matrix, 0.1, 0.99, 1.0, 1.0, verbose=0)
            push!(times, t)
    ...
    @testset "Numerical Stability Analysis" begin
        ...
        for s in stretch_factors
            ...
            pol = Globtim.MainGenerate(f, n, grid_matrix, 0.1, 0.99, [s, 1.0], 1.0, verbose=0)
    ...
    @testset "Integration with Existing Features" begin
        ...
        pol_float = Globtim.MainGenerate(f, n, grid_matrix, 0.1, 0.99, 1.0, 1.0, 
                                        precision=Globtim.FloatPrecision, verbose=0)
        @test pol_float.precision == Globtim.FloatPrecision
        
        pol_rat = Globtim.MainGenerate(f, n, grid_matrix, 0.1, 0.99, 1.0, 1.0, 
                                       precision=Globtim.RationalPrecision, verbose=0)
        @test pol_rat.precision == Globtim.RationalPrecision
        
        # Test with normalization
        pol_norm = Globtim.MainGenerate(f, n, grid_matrix, 0.1, 0.99, 1.0, 1.0, 
