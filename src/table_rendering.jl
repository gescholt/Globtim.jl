...
function render_console_table(stats_table::ComprehensiveStatsTable; width=80)
    ...
    if ca.total_count > 0
        ...
        push!(lines, format_table_row("Excellent (< 1e3)", 
            "$(ca.excellent_count) ($(round(100*ca.excellent_count/total, digits=1))%)", table_width))
        push!(lines, format_table_row("Good (1e3-1e6)", 
            "$(ca.good_count) ($(round(100*ca.good_count/total, digits=1))%)", table_width))
        push!(lines, format_table_row("Fair (1e6-1e9)", 
            "$(ca.fair_count) ($(round(100*ca.fair_count/total, digits=1))%)", table_width))
        push!(lines, format_table_row("Poor (1e9-1e12)", 
            "$(ca.poor_count) ($(round(100*ca.poor_count/total, digits=1))%)", table_width))
        push!(lines, format_table_row("Critical (≥ 1e12)", 
            "$(ca.critical_count) ($(round(100*ca.critical_count/total, digits=1))%)", table_width))
    ...
    has_validation_data = !ismissing(vr.eigenvalue_signs_correct) || 
    ...
    if has_validation_data
        ...
        if stats_table.point_type == :minimum
            if !ismissing(vr.eigenvalue_signs_correct)
                ...
                push!(lines, format_table_row("All eigenvalues positive", status, table_width))
            end
            if !ismissing(vr.positive_eigenvalue_count)
                push!(lines, format_table_row("Positive eigenvalue count", 
                    string(vr.positive_eigenvalue_count), table_width))
            end
        ...
        elseif stats_table.point_type == :maximum
            if !ismissing(vr.eigenvalue_signs_correct)
                status = format_validation_value(vr.eigenvalue_signs_correct)
                push!(lines, format_table_row("All eigenvalues negative", status, table_width))
            end
            if !ismissing(vr.negative_eigenvalue_count)
                push!(lines, format_table_row("Negative eigenvalue count", 
                    string(vr.negative_eigenvalue_count), table_width))
            end
            if !ismissing(vr.determinant_sign_consistent)
                status = format_validation_value(vr.determinant_sign_consistent)
                push!(lines, format_table_row("Determinant sign consistent", status, table_width))
            end
        ...
        elseif stats_table.point_type == :saddle
            if !ismissing(vr.mixed_eigenvalue_signs)
                ...
                push!(lines, format_table_row("Mixed eigenvalue signs", status, table_width))
            end
        end
        ...
        for (key, value) in vr.additional_checks
            if isa(value, Number) && isfinite(value)
                ...
                display_value = isa(value, AbstractFloat) ? 
                    string(round(value, digits=4)) : string(value)
    ...
    if !ismissing(stats_table.eigenvalue_stats)
        ...
        if es.count > 0
            ...
            push!(lines, format_table_row("Eigenvalue count", string(es.count), table_width))
            push!(lines, format_table_row("Mean ± Std", 
                "$(round(es.mean, digits=6)) ± $(round(es.std, digits=6))", table_width))
            push!(lines, format_table_row("Range", 
                "[$(round(es.min, digits=6)), $(round(es.max, digits=6))]", table_width))
        end
    end
...
function render_comparative_table(stats_list::Vector{ComprehensiveStatsTable}; width=100)
    ...
    for stats in stats_list
        ...
        if count > 0
            ...
            valid_pct = if stats.point_type == :minimum && !ismissing(vr.eigenvalue_signs_correct)
                vr.eigenvalue_signs_correct ? 100.0 : 0.0
            elseif stats.point_type == :maximum && !ismissing(vr.eigenvalue_signs_correct)
                vr.eigenvalue_signs_correct ? 100.0 : 0.0
    ...
    if total_points > 0
        ...
        push!(lines, create_table_border(table_width, :middle))
        push!(lines, format_table_row("Total critical points", string(total_points), table_width))
        push!(lines, format_table_row("Overall numerical quality", 
            overall_well_cond > 80 ? "EXCELLENT" : overall_well_cond > 60 ? "GOOD" : "FAIR", table_width))
        push!(lines, format_table_row("Mathematical validation", "$(overall_valid)% pass rate", table_width))
        push!(lines, format_table_row("Production readiness", 
            (overall_well_cond > 70 && overall_valid > 80) ? "READY" : "NEEDS REVIEW", table_width))
    end
