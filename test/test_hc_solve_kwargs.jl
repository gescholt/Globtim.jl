@testset "HC solve kwargs (sparsify + polyhedral)" begin
    ...
    cps_poly = solve_polynomial_system(x, n, d, pol.coeffs;
        basis=:chebyshev, normalized=true, start_system=:polyhedral)
    @test length(cps_poly) >= 1

    # ── 3. Sparsification + polyhedral
    cps_sparse = solve_polynomial_system(x, n, d, pol.coeffs;
        basis=:chebyshev, normalized=true,
        sparsify_threshold=1e-6, start_system=:polyhedral)
    @test length(cps_sparse) >= 1

    # ── 4. Explicit :total_degree still works for 3D
    cps_td = solve_polynomial_system(x, n, d, pol.coeffs;
        basis=:chebyshev, normalized=true, start_system=:total_degree)
    @test length(cps_td) >= 1

    # ── 5. solve_polynomial_system_from_approx forwards kwargs
    cps_approx = Globtim.solve_polynomial_system_from_approx(x, pol;
        sparsify_threshold=1e-6, start_system=:polyhedral)
