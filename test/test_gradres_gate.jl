...
@testset "GRADRES-GATE: recovered-minimizer plateau" begin
    ...
    @test Globtim._recovered_minimizer_plateaued(plateaued, 1e-3)

    # Minimizer still moving by ~10% each step ⇒ DO NOT stop.
    moving = [mk(4, [1.0, 2.0]), mk(6, [1.1, 2.1]), mk(8, [1.2, 2.2])]
    @test !Globtim._recovered_minimizer_plateaued(moving, 1e-3)
    ...
    @test !Globtim._recovered_minimizer_plateaued(one_step, 1e-3)

    # Fewer than 3 successful minimizers ⇒ cannot conclude ⇒ false.
    @test !Globtim._recovered_minimizer_plateaued([mk(4, [1.0]), mk(6, [1.0])], 1e-3)
    ...
    @test Globtim._recovered_minimizer_plateaued(holes, 1e-3)
    ...
    @test Globtim._recovered_minimizer_plateaued(with_failures, 1e-3)
end
