...
function validate_project_structure()
    ...
    if project_root !== nothing
        ...
        all_good &= check_mark(
            isfile(joinpath(project_root, "environments", "local", "Project.toml")),
            "Local environment Project.toml exists")
        all_good &= check_mark(
            isfile(joinpath(project_root, "environments", "hpc", "Project.toml")),
            "HPC environment Project.toml exists")
        all_good &= check_mark(isdir(joinpath(project_root, ".globtim")),
            ".globtim/ directory exists")
        all_good &=
            check_mark(isfile(joinpath(project_root, ".globtim", "notebook_setup.jl")),
...
function test_universal_include()
    ...
    try
        ...
        universal_path = joinpath(
            dirname(Base.find_package("Globtim")),
            "..",
            ".globtim",
            "notebook_setup.jl"
        )
        all_good &=
            check_mark(isfile(universal_path), "Universal include path resolves correctly")
...
function provide_troubleshooting_info()
    ...
    for var in
        ["SLURM_JOB_ID", "PBS_JOBID", "GLOBTIM_FORCE_PLOTTING", "GLOBTIM_ENV", "DISPLAY"]
