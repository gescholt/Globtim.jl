...
@testset "Grid-based MainGenerate" begin
    ...
    @testset "Grid validation" begin
        ...
        @test_throws AssertionError Globtim.MainGenerate(f, n, grid_wrong_dim, 0.1, 0.99, 1.0, 1.0)
        
        # Test empty grid
        empty_grid = Matrix{Float64}(undef, 0, 2)
        @test_throws AssertionError Globtim.MainGenerate(f, n, empty_grid, 0.1, 0.99, 1.0, 1.0)
    end
    ...
    @testset "Different basis types with grid" begin
        ...
        # Test with Chebyshev
        pol_cheb = Globtim.MainGenerate(f, n, grid, 0.1, 0.99, 1.0, 1.0, basis=:chebyshev, verbose=0)
        @test pol_cheb.basis == :chebyshev
        
        # Test with Legendre
        pol_leg = Globtim.MainGenerate(f, n, grid, 0.1, 0.99, 1.0, 1.0, basis=:legendre, verbose=0)
    ...
    @testset "Performance comparison" begin
        ...
        t1 = @elapsed pol1 = Globtim.MainGenerate(f, n, d, 0.1, 0.99, 1.0, 1.0, GN=GN, verbose=0)
        
        # Time with pre-generated grid
        t2 = @elapsed pol2 = Globtim.MainGenerate(f, n, grid_matrix, 0.1, 0.99, 1.0, 1.0, verbose=0)
