# ================================================================================
# 4D Deuflhard - 16 Subdivisions Adaptive Convergence Analysis
# ================================================================================
# 
# Advanced spatial analysis with fixed L²-tolerance and adaptive degree/sample increase
# per subdomain until convergence is achieved
#
# Key Improvements:
# - Fixed L²-tolerance target across all subdomains
# - Adaptive degree increase per subdomain until L²-norm target is met
# - Dynamic sample count adjustment when needed
# - Enhanced convergence tracking and visualization
# - Robust error handling and timeout management
#
# Objectives:
# - Achieve consistent L²-norm quality across all 16 subdomains
# - Identify computational requirements per spatial region
# - Generate convergence progression plots for each subdomain
# - Analyze spatial difficulty patterns across the 4D domain
#
...
- `gn`: Sample count parameter (GN)
- `subdomain`: Subdomain structure defining spatial region
- `theoretical_points`: Reference critical points within subdomain

# Returns
- `NamedTuple`: Comprehensive metrics for the subdomain at specified degree and sample count
"""
...
# Returns
- `Vector{NamedTuple}`: Analysis results for convergence progression

# Description
Adaptively increases degree and sample count until L²-norm target is achieved
or maximum limits are reached.
"""
...
"""
    plot_combined_adaptive_convergence(all_subdivision_results::Dict)

Create combined L²-norm convergence plot showing adaptive progression for all 16 subdomains.

# Arguments
- `all_subdivision_results`: Dictionary mapping subdomain labels to results arrays

# Returns
- `Figure`: Combined plot with 16 adaptive convergence curves
"""
function plot_combined_adaptive_convergence(all_subdivision_results::Dict)
    @info "Starting combined adaptive convergence plot generation"
    ...
    hlines!(ax, [L2_TOLERANCE_TARGET], color = :black, linestyle = :dash, linewidth = 2, label = "Target L²-Norm")
    
    # Add grid
    ax.xgridvisible = true
    ax.ygridvisible = true
    
    # Legend below plot (avoid GLMakie text issues)
    if plotted_count > 0
        try
            Legend(fig[2, 1], ax, orientation = :horizontal, nbanks = 4, framevisible = true)
        catch e
            @warn "Legend creation failed, skipping" exception=e
        end
    end
    ...
    @info "Adaptive convergence plot generation complete" plotted_subdivisions=plotted_count
    return fig
end
...
@info "Starting 4D Deuflhard 16-Subdivision Adaptive Analysis" timestamp=Dates.format(now(), "yyyy-mm-dd HH:MM:SS") target_l2=L2_TOLERANCE_TARGET max_degree=DEGREE_MAX
...
output_dir = joinpath(@__DIR__, "outputs", "adaptive_subdivisions_$(Dates.format(now(), "yyyy-mm-dd_HH-MM"))")
mkpath(output_dir)
@info "Created output directory" path=output_dir

# Generate combined visualization (save to file only)
@info "Generating combined adaptive convergence plot (file output only)"
try
    ...
    if !isempty(non_empty_results)
        fig_combined = plot_combined_adaptive_convergence(all_subdivision_results)
    ...
    @warn "Adaptive plotting failed: $e"
