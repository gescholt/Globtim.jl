...
function Globtim.plot_all_eigenvalues(f::Function, df::DataFrames.DataFrame; sort_by=:magnitude)
    ...
    for ptype in point_types
        ...
        if !isempty(type_mask)
            ...
            if sort_by == :magnitude
                type_indices[ptype] = sort(type_mask, by=i -> maximum(abs.(all_eigenvalues[i])), rev=true)
            ...
            elseif sort_by == :largest
                type_indices[ptype] = sort(type_mask, by=i -> maximum(all_eigenvalues[i]), rev=true)
            elseif sort_by == :spread
                type_indices[ptype] = sort(type_mask, by=i -> maximum(all_eigenvalues[i]) - minimum(all_eigenvalues[i]), rev=true)
            else  # :index
    ...
    available_types = [ptype for ptype in point_types if haskey(type_indices, ptype) && !isempty(type_indices[ptype])]
    ...
    fig = CairoMakie.Figure(size=(1400, 400 * n_types))
    
    # Determine plot labels based on sort option
    y_label = sort_by == :abs_magnitude ? "Eigenvalue Magnitude (Absolute Value)" : "Eigenvalue Magnitude"
    ...
    for (subplot_idx, ptype) in enumerate(available_types)
        ...
        plot_title = sort_by == :abs_magnitude ? 
        ...
        ax = CairoMakie.Axis(fig[subplot_idx, 1], 
        ...
        for (plot_idx, orig_idx) in enumerate(sorted_indices)
            ...
            CairoMakie.lines!(ax, fill(x_pos, length(plot_vals)), plot_vals, 
                     color=type_color, linestyle=:dot, linewidth=1, alpha=0.7)
            
            # Plot individual eigenvalues
            for (eig_idx, (eigenval, plot_val)) in enumerate(zip(eigenvals, plot_vals))
                CairoMakie.scatter!(ax, [x_pos], [plot_val], 
        ...
        if sort_by != :abs_magnitude
            CairoMakie.hlines!(ax, [0], color=:black, linestyle=:dash, alpha=0.5)
        end
    end
    
    # Create eigenvalue legend
    eigenval_legend_elements = [CairoMakie.MarkerElement(color=eigenval_colors[i], marker=:circle, markersize=10) 
    ...
    type_legend_elements = [CairoMakie.MarkerElement(color=:black, marker=:circle, markersize=10, strokecolor=type_colors[ptype], strokewidth=2) 
    ...
    CairoMakie.Legend(fig[1:n_types, 2], eigenval_legend_elements, eigenval_legend_labels, 
                     "Eigenvalue Order", tellheight=false, framevisible=true)
    CairoMakie.Legend(fig[1:n_types, 3], type_legend_elements, type_legend_labels, 
                     "Critical Point Type", tellheight=false, framevisible=true)
    
    return fig
end

function Globtim.plot_raw_vs_refined_eigenvalues(f::Function, df_raw::DataFrames.DataFrame, df_refined::DataFrames.DataFrame; sort_by=:euclidean_distance)
    ...
    # Create figure with subplots for each type
    fig = CairoMakie.Figure(size=(1400, 600 * n_types))
    ...
    for (subplot_idx, ptype) in enumerate(available_types)
        ...
        for (pair_idx, (raw_idx, refined_idx, distance)) in enumerate(type_matches)
            ...
            for (eig_idx, eigenval) in enumerate(raw_sorted[1:n_eigenvals])
                ...
                CairoMakie.scatter!(ax, [x_pos], [y_pos], 
                          color=eigenval_colors[min(eig_idx, length(eigenval_colors))],
                          marker=:circle,
                          markersize=8,
                          strokecolor=type_color,
                          strokewidth=1.5,
                          alpha=0.6)  # Lighter for raw points
            end
            ...
            for (eig_idx, eigenval) in enumerate(refined_sorted[1:n_eigenvals])
                ...
                CairoMakie.scatter!(ax, [x_pos], [y_pos], 
                          ...
                          strokecolor=type_color,
                          strokewidth=1.5,
                          alpha=1.0)  # Darker for refined points
            end
            ...
            for eig_idx in 1:n_eigenvals
                ...
                CairoMakie.lines!(ax, [x_pos, x_pos], [raw_y, refined_y],
                         color=eigenval_colors[min(eig_idx, length(eigenval_colors))],
                         linestyle=:solid,
                         linewidth=1.5,
                         alpha=0.7)
            ...
            if distance > 0
                CairoMakie.text!(ax, x_pos, minimum(refined_sorted) + refined_y_offset - 0.1,
        ...
        CairoMakie.hlines!(ax, [0], color=type_color, linestyle=:solid, alpha=0.3, linewidth=2)
    end
    ...
    raw_refined_elements = [
        CairoMakie.MarkerElement(color=:blue, marker=:circle, markersize=10, alpha=0.6),
        CairoMakie.MarkerElement(color=:blue, marker=:circle, markersize=10, alpha=1.0)
    ]
    ...
    CairoMakie.Legend(fig[1:n_types, 2], eigenval_legend_elements, eigenval_legend_labels, 
                     "Eigenvalue Order", tellheight=false, framevisible=true)
    CairoMakie.Legend(fig[1:n_types, 3], raw_refined_elements, raw_refined_labels, 
                     "Point Type", tellheight=false, framevisible=true)
